...
- Current stage: `ENGINEER (M6 alignment fix complete; import not verified)`
- Next owner: `Human/QA` (`M6 — workflow import & credential binding`; menunggu owner account, runtime values, dan approval)
...
| Engineer handoff | `workflows/01-corpus-ingestion.json` (m6-aligned-v1), `workflows/02-telegram-grounded-qa.json` (m6-aligned-v1), `tests/harness/*`, `tests/mocks/ai-provider/*`, `tests/fixtures/*`, `evaluation/qa-dataset.csv` (12+3), `README.md`, `.ai/reports/build/M5-final-traceability.md`, `.ai/reports/build/M6-workflow-schema-credential-branching-alignment-report.md` | `M0-M6 alignment / 2026-09-15` | `VERIFIED BY IMPLEMENTER` | M0-M5 regression remains 67/67; M6 alignment adds schema migration, credential-store references, corrected branching, and timing/state fields; workflow exports remain DRAFT unpublished (import NOT_VERIFIED) |
| DevOps handoff | `deploy/compose.yaml`, `deploy/postgres-init/01-init.sh`, `deploy/start-local.sh`, `deploy/stop-local.sh`, `.env.test`, `deploy/WORKFLOW-IMPORT-GUIDE.md` | `M6 / 2026-09-14` | `VERIFIED BY IMPLEMENTER` | local-isolated profile: n8n 1.62.1 + pgvector pg16 BOTH healthy; DB init verified (3 roles, 2 schemas, 5 tables, pgvector 0.8.6); permission matrix PASS; loopback-only verified; workflow import NOT_YET (menunggu credential/owner) |
| Build / self-test evidence | `.ai/reports/build/M1-safe-isolated-foundation-exit-report.md`, `M2-atomic-corpus-ingestion-exit-report.md`, `M3-grounded-answer-core-exit-report.md`, `M4-telegram-delivery-deadline-exit-report.md`, `M5-portable-delivery-bundle-exit-report.md`, `M6-local-isolated-deployment-exit-report.md`, `M6-workflow-schema-credential-branching-alignment-report.md` | `2026-09-15` | `VERIFIED BY IMPLEMENTER` | M1 (foundation, static validation); M2 (AC-001..005, 19/19); M3 (AC-011..020, 21/21); M4 (AC-006..010 + AC-022..024, 27/27); M5 (finalization, 67/67 total); M6 deployment plus alignment static contract pass. All self-tests local-isolated, no real credentials/network/real Telegram QA |
...
- From stage / owner: `Engineer (M6 alignment fix complete)` / `M6 — workflow import, credential binding & runtime integration`
- Changed or produced artifacts: `workflows/01-corpus-ingestion.json` (`m6-aligned-v1`), `workflows/02-telegram-grounded-qa.json` (`m6-aligned-v1`), `deploy/postgres-init/01-init.sh`, `deploy/postgres-init/02-rag-schema-alignment.sql`, `deploy/postgres-init/README.md`, `deploy/WORKFLOW-IMPORT-GUIDE.md`, `README.md`, `evaluation/README.md`, `.ai/reports/build/M6-workflow-schema-credential-branching-alignment-report.md`.
- Evidence and result: both workflow JSONs parse; all 45 nodes reachable; credential contract PASS; no `$env` or literal secrets in exports; compose config `--quiet` PASS; M2/M3/M4 regression 67/67 PASS. Existing local stack/DB evidence remains from the DevOps handoff.
- Bugs fixed: (1) workflow/schema legacy column mismatch; (2) missing existing-volume schema migration; (3) `$env` credential boundary conflict; (4) missing input/dedup/abstention/failure/delivery stop branches; (5) activation could supersede the active corpus before confirming a candidate; (6) delivery timing/state fields were not populated; (7) stale import/evaluation instructions.
- Assumptions / untested items: workflow import into n8n (AC-025) NOT yet done — waits for owner account + named credentials; Telegram webhook/send, provider profiling, live DB transaction behavior, and QA remain untested; demo-vps profile requires Human provisioning.
- Required next action: Human/QA — import both workflows, bind credentials, run schema alignment on an existing volume if applicable, populate approved `rag.rag_settings`, run workflow 01, and report import/runtime failures; then Engineer calibration (M7).
