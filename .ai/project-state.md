...
- Current stage: `ENGINEER COMPLETE THROUGH M6 (runtime readiness verified by implementer; M7/M8 not started)`
- Next owner: `Human` (`select the independent M7 QA/Security handoff; M7/M8 do not start automatically`)
...
| M6 — Runtime & test-target readiness | `VERIFIED BY IMPLEMENTER — READY FOR QA` | n8n/PostgreSQL/Ollama Gemma binding, live ingestion, webhook route, temporary HTTPS path, cleaned 34-node workflow, and container-to-Gemma warm request are verified. Cold-start latency and independent acceptance remain explicit M7/operational risks. |
...
| Engineer handoff | `workflows/01-corpus-ingestion.json` (m6-live-ingestion-v2), `workflows/02-telegram-grounded-qa.json` (m6-gemma-e2b-clean-v1), `tests/harness/*`, `tests/mocks/ai-provider/*`, `tests/fixtures/*`, `evaluation/qa-dataset.csv` (12+3), `README.md`, `.ai/reports/build/M5-final-traceability.md`, `.ai/reports/build/M6-workflow-schema-credential-branching-alignment-report.md`, `.ai/reports/build/M6-local-ollama-provider-readiness-report.md`, `.ai/reports/build/M6-live-corpus-ingestion-runtime-fix-report.md` | `M0-M6 local provider / 2026-09-20` | `VERIFIED BY IMPLEMENTER` | Workflow 01 live path reaches atomic activation: 26 documents, 297 persisted chunks, dimension 768; identical rerun inserts 0 duplicate rows. Workflow 02 is active with Gemma binding, 34 nodes, and clean graph validation; semantic QA remains outside Engineer verdict |
...
- Assumptions / untested items: live invalid/timeout/provider/database/concurrency faults were not rerun; previous Telegram smoke telemetry contains 19 successes and 3 `delivery_unknown`; grounded answer/source quality was not independently scored; cold-start latency remains exploratory; the Quick Tunnel is not production infrastructure; QA, Security, latency 15/15, demo-vps, M7, and M8 remain untested/out of scope.
- Required next action: Human selects whether to hand the exact Gemma-bound candidate to independent M7 QA/Security. Do not start M7/M8 automatically and do not treat the warm probe as the required 15-item acceptance benchmark.
