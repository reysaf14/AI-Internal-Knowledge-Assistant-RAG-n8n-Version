...
- Current stage: `ENGINEER (M6 local Ollama runtime settings applied; ingestion not verified)`
- Next owner: `Human/QA` (`M6 — run corpus ingestion and verify active corpus`; credential binding/import sudah dilakukan Human, host binding dan live execution masih perlu diverifikasi)
...
- Required next action: Human/QA — re-import the updated workflow 01 (including filename fallback and failure-reason propagation), then run it and inspect the first failing node; verify an `active` corpus only after the upstream failure is resolved. M7/M8 remain out of scope for this handoff.
